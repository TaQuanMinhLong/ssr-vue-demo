...
//   const inlined = html.replace(
//     reStyle,
//     (_, beforeSrc, afterSrc) => `<style${beforeSrc}${afterSrc}>${newCode.trim()}</style>`,
//   );
//   return inlined;
// }
  ...
  return {
    ...
    generateBundle: (_, bundle) => {
      ...
      for (const id in bundle) {
        if (isHtmlFile.test(id)) {
          files.html.push(id);
        }
        // else if (isCssFile.test(i)) {
        //   files.css.push(i);
        // }
        else if (isJsFile.test(id)) {
          const chunk = bundle[id] as OutputChunk;
          if (!chunk.facadeModuleId?.includes("/templates/")) {
            files.js.push(id);
          }
        } else {
          files.other.push(id);
